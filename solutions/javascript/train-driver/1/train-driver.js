...
export function fixListOfWagons(ids) {
  const [first, second, ...restNums] = ids;
  return [...restNums, first, second];
}
...
export function correctListOfWagons(ids, missingWagons) {
  const [first, ...restNums] = ids;
  return [first, ...missingWagons, ...restNums];
}
...
export function separateTimeOfArrival(information) {
  const {timeOfArrival, ...routing} = information;
  return [timeOfArrival, routing];
}
